...
async def add_link_to_lesson(session_maker: async_sessionmaker, group_id: int,
                             lesson_name: str, lesson_link: str) -> None:
    async with session_maker() as session:
        async with session.begin():

            # У тебя 2 ключа - группа и урок, ссылку можно менять
            ...
            if link:
                ...
                set_link = LessonsLinks(group_id=group_id, lesson_name=lesson_name, lesson_link=lesson_link)
                await session.merge(set_link)
                return


async def get_link(session_maker: async_sessionmaker, group_id: int, lesson_name: str) -> str:
    async with session_maker() as session:
        async with session.begin():
        ...
        async with session.begin():
            ...
            if links_list:
                for link in links_list:
                    clear_links_list.append([link[0].lesson_name, link[0].lesson_link])
