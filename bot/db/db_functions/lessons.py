...
async def add_lesson(session_maker: async_sessionmaker, group_id: int, subgroup: int,
                     lesson_name: str, lesson_time_start: str, lesson_date_start: str) -> None:
    async with session_maker() as session:
        async with session.begin():
            # await session.execute(select(Lessons))
            lesson = Lessons(group_id=group_id, subgroup=subgroup, lesson_name=lesson_name,
                             lesson_time_start=lesson_time_start, lesson_date_start=lesson_date_start)
        ...
        async with session.begin():
            ...
            if result:
                ...
                for lesson in lessons:
                    groups_to_send.append([lesson[0].group_id, lesson[0].subgroup, lesson[0].lesson_name])
